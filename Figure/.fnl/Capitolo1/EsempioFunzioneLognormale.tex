...
\begin{tikzpicture}
    ...
    %\begingroup Funzioni
        \pgfmathsetmacro\m1\pgfmathsetmacro\lasts{0.04}
        \newcommand\lognormalX{(1/(x*log(10)*s*sqrt(2*pi)))*exp(-(log10(x)-m)**2/(2*s**2))}
        \newcommand\normal{(1/(s*sqrt(2*pi)))*exp(-(x-m)**2/(2*s**2))}

        \newcommand\plotFunction[1]{
            \foreach \s in {1.12,1,0.85,0.65,0.45,0.25,0.15,0.08,0.05,\lasts}{%
                \addplot+ [thick,no marks] gnuplot [raw gnuplot] {%
                    set samples 5000; m=\m; s=\s; #1
                };
        ...
        \newcommand\plotLognormalAverage{
            \addplot [draw=none,forget plot] gnuplot [raw gnuplot] {%
                f(x,m,s)=\lognormalX; m=\m; s=\lasts;
                plot[x=10**(\m):10**(\m)] (f(x,m,s));
            } coordinate (media); 
            \draw [line width=.5pt,dashed,black] (axis cs:{10^\m},0) -- (media);
        }
    ...
    \begin{groupplot}[
        ...
        %\begingroup
            ...
            \nextgroupplot[%
                legend style={
                    \placeLegend{0}{1}{north west}{5em}{0em},
