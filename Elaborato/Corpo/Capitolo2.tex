...
\chapter{Note di teoria dei grafi}\label{secTeoriaGrafi}
    ...
    \section{Definizioni miscellanee}\label{secDefinizioniGrafi}
        ...
        \begin{dfn}[Grafo]
            \label{defGrafo}
            Un grafo è un insieme \(\mathcal G=(\mathcal I,\mathcal E)\) formato dalla coppia dell'insieme degl'indici dei nodi \(\mathcal I\) e dell'insieme dei lati \(\mathcal E\), vale a dire di coppie d'indici \(\mathcal I\): due nodi \(i,j\in\mathcal I\) sono connessi sse \((i,j)\in\mathcal E\).
        \end{dfn}
        ...
        \begin{dfn}[Grafo \textnormal{[in]}diretto]
            ...
            Un grafo è indiretto sse \[%
                (i,j)\in\mathcal E\ \ \iff\ \ (j,i)\in\mathcal E, %\forall(i,j)\in\mathcal E,
            \] e la direzione è trascurabile; altrimenti è detto diretto.
        \end{dfn}
        
        La trascurabilità della direzione sarà discussa poco dopo nel \cref{secRetiECittà}, anche se è facilmente intuibile dalla \cref{figEsempioGrafoInDiretto}.

        % \tikzfigure{H}
        %     {\input{../../Figure/.fnl/Capitolo2/EsempioGrafoInDiretto.tex}}
        \tikzexternalizedfigure{H}
            {../../Figure/.fnl/Capitolo2/EsempioGrafoInDiretto.tex}
            {Esempio di un grafo indiretto (a), della sua equivalente forma diretta (b) e della loro [identica] matrice d'adiacenza (c).}
            {.95\linewidth}{figEsempioGrafoInDiretto}
        
        \begin{dfn}[Matrice d'adiacenza]
            \label{defMatriceAdiacenzaUnitariaPesata}
            \newcommand\hspz{\hspace{1em}}
            Sia \(N\equiv\vert\mathcal I\vert\in\NPlus\) la cardinalità dell'insieme degl'indici\footnotemark{}, ossia il numero di vertici totali, si definisce la matrice d'adiacenza \(\mathbf M\in\mathbb R^{N\times N}\) pesata come:%
            \footnotetext{Per \(\NPlus\) si veda la \cref{defSemirettaInteraRealePositiva} poco dopo.}%
            %
            \begin{equation}
            ...
            \end{equation}
            %
            Si osserva che, se non altrimenti detto, scrivendo solo \textit{matrice d'adiacenza} si sottintende sempre quella unitaria.
        \end{dfn}

        \begin{dfn}[Matrice trasposta]
            Data \(\mathbf M\in\mathbb R^{N\times N}\), con \(N\in\NPlus\), s'indica con \(\mathbf M^\top\) la matrice trasposta cosí definita:
            %
            \begin{equation*}
        ...
        \begin{dfn}[Matrice simmetrica]
            Data \(\mathbf M\in\mathbb R^{N\times N}\), con \(N\in\NPlus\), essa è simmetrica sse
            %
            \begin{equation}
        ...
        \begin{dfn}[Gradi e Forze]
            ...
            \begin{equation}
                ...
                w^-_i\equiv\sum_{j=1}^Nm_{j,i}%=(\mathbf M^\top\mathbf 1)_i
                \quad\text{e}\quad
                w^+_i\equiv\sum_{j=1}^Nm_{i,j},%=(\mathbf M\mathbf 1)_i,
            ...
            \begin{equation}
                ...
                k^-_i\equiv\sum_{j=1}^Na_{j,i}%=(\mathbf A^\top\mathbf 1)_i
                \quad\text{e}\quad
                k^+_i\equiv\sum_{j=1}^Na_{i,j},%=(\mathbf A\mathbf 1)_i,
                \quad\forall i\in\mathcal I,
            \end{equation}
            %
    ...
    \section{Reti e città}\label{secRetiECittà}
        ...
        Dopo aver illustrato un po' di teoria sui grafi sorge successivamente il problema di come rappresentare con essi la moltitudine di collegamenti urbani; in effetti, vi sono molti modi di rappresentare le città mediante i grafi: i primi si pongono a livello \textit{intraurbano}, o considerando le strade come lati e le loro intersezioni come nodi \cite[§ 3.1.3.1 p. 17]{Barthelemy2011}, o rappresentando le reti di trasporto (tram, bus e metro) \cite[§ 3.2.1 p. 22]{Barthelemy2011}; altri si pongono piú propriamente a livello \textit{interurbano} prendono singolarmente in considerazione varie reti dei trasporti (ferroviario \cite[§ 3.1.3.2 p. 17]{Barthelemy2011}, navale \cite[§ 3.1.4 p. 19]{Barthelemy2011}, aereo \cite[§ 3.1.2 p. 13]{Barthelemy2011}, ecc.).

        Tuttavia, da quanto detto nel \cref{secIntroduzione}, è chiaro che per predire la distribuzione della popolazione tra città valgono le seguenti osservazioni:
        ...
        \begin{enumerate}[
            ...
            \item ogni rappresentazione intraurbana va scartata perché è troppo fine, oltre a considerare movimenti limitatamente a una sola città;
        ...
        \begin{enumerate}[
            ...
            \item ammesso che una città possa interagire con un'altra, allora è sempre possibile l'opposto: le potenziali interazioni sono simmetriche.
        \end{enumerate}
    
        Dunque il grafo in questione è diretto ma con struttura indiretta (\cref{defGrafoInDiretto}):

        \begin{ipo}[\(\mathbf A\) simmetrica]
            \label{ipTipoGrafo}
            Il grafo \(\mathcal G\) è diretto e simmetrico.
        \end{ipo}
    ...
    \section{Cenni sui dati}\label{secCenniSuiDati}
        ...
        \begin{table}[htb]
            ...
            \begin{NiceTabular}{ccc}
                ...
                % \Body
                    ...
                    \hline
                \CodeAfter
                    \tikz \foreach \r in {4,...,8} \draw[%
                        color=white!40!black,
                        line width=.5pt,
                    ] (\r.5-|1) -- (\r.5-|4);
                    \tikz \node[anchor=east] at (11.5-|1) {\Circled{\(\ast\)}};
            \end{NiceTabular}
            \label{tabStrutturaDatiPendolarismo}
        \end{table}
        \footnotetext{si v. il documento \texttt{trapen91.txt} per maggiori informazioni.}

        I dati considerano tutt'i comuni italiani nel 1991 (in totale 8100), quindi è necessario estrapolare da essi le matrici di pendolarismo delle singole regioni e parallelamente quella complessiva dell'Italia. In piú, si possono definire due tipi di matrici d'adiacenza:
        ...
        \begin{enumerate}[
            ...
            \item quella unitaria in cui un lato esiste se almeno un pendolare (cioè la riga \Circled{\(\ast\)} nella \cref{tabStrutturaDatiPendolarismo}) si muove tra i due comuni e
            \item quella pesata che è analoga alla precedente ma con peso \(q_{i,j}\) uguale alla somma totale dei pendolari tra due comuni.
        \end{enumerate}
        ...
        \begin{enumerate}[
            ...
            \item in molte linee come comune di destinazione sono segnati dei codici incompleti contenenti solo l'ultima metà (codice comune) ma non la prima (codice provincia), e questi sono in totale 8: «215», «229», «241», «216», «203», «224», «236» e «246».
        \end{enumerate}
        ...
        \begin{oss}
            A dire il vero si potrebbe scegliere, tra i comuni con uguale codice comunale, quello che minimizza la distanza euleriana, visto che l'ISTAT \cite{ISTATLimiti1991} rende disponibili le coordinate dei punti rappresentativi di ogni comune. In ogni caso il numero d'eccezioni è cosí esiguo da rendere una tale "correzione" del tutto innecessaria\footnotemark{}\footnotetext{I risultati del \cref{secSimulazioni} mostrano anche che è irrilevante.}. Il tutto senza considerare che la scelta piú saggia è banalmente quella di prendere dati piú recenti\footnotemark{}\footnotetext{Non si è fatto per riprodurre \cite{DeMontis2007}, com'è discusso súbito dopo.}.
        \end{oss}

        % \tikzfigure{H}
        %     {\input{../../Figure/.fnl/Capitolo2/AnalisiRete20/SVsD.tex}}
        \tikzexternalizedfigure{H}
            {../../Figure/.fnl/Capitolo2/AnalisiRete20/SVsD.tex}
            {Forza contro grado della Sardegna.}
            {.75\linewidth}{figForzaVsGrado20}

        Infine nelle \cref{figForzaVsGrado20,figDeMontisRiproduzione} sono stati anche riprodotti i risultati di \cite{DeMontis2007} anche se con alcune differenze:
        ...
        \begin{enumerate}[
            ...
            \item la \cref{figDeMontisRiproduzione}(c) rispetto alla \cite[Fig. 3b]{DeMontis2007} presenta sia una forma leggermente diversa, seppure la tendenza a mezza luna sia la stessa, che limiti superiori e inferiori piú estesi;
            \item il coefficiente d'aggregazione pesato nella \cref{figDeMontisRiproduzione}(g) ha chiaramente una tendenza decrescente, anche se poco ripida per cui si può considerare comunque «circa costante» come afferma \cite{DeMontis2007};
            \item i primi due pesi piú elevati, riportati nella \cref{tabConfrontoPesiDeMontis}, non corrispondono, ma è probabilmente un refuso da parte di \cite{DeMontis2007} poiché i collegamenti «Cagliari-Sassari» e «Sassari-Olbia» sono molto distanti geograficamente\footnote{Il primo equivale a percorrere l'intera estensione dell'isola ogni giorno.}, per cui è ragionevole che i flussi siano bassi.
        \end{enumerate}

        Queste discrepanze sono trascurabili perché lievi e, per quanto concerne questo elaborato, non significative. Inoltre l'unico risultato di rilievo, usato nel \cref{secInterpretazioni}, è la correlazione positiva tra la forza e il grado nella \cref{figForzaVsGrado20}.
        ...
        \tikzexternalizedfigure{p}%[-1em]
            {../../Figure/.fnl/Capitolo2/AnalisiRete20/Infopagina.tex}
            {Riproduzione dei principali grafici di \protect\cite{DeMontis2007} [a cui si rimanda per maggiori dettagli sui vari coefficienti] relativi alla topologia della rete sarda del pendolarismo.}
