...
%\begingroup Codice
    %\begingroup Minted
        ...
        \newcommand\vTextSep{\dimColla{1}{1}{0}{em}}
        \newcommand\vCaptionSep{2pt}
        \newcommand\vBodySep{0pt}
        ...
        \newenvironment{code}{
            ...
            \par\addvspace{\vTextSep}
        }{\par\addvspace{\vTextSep}}

        \newcommand\printListing[5][python]{
            \begin{code}
                \pdfbookmark[3]{#4}{#5}
...
%\begingroup Figure
    ...
    \newcommand\oddevenpagefloat[2][0]{%
        \ifcase#1 % Figura posizionata nella prima pagina sinistra (numero pari)
            \ifodd\value{page}
                #2
            \else
                \afterpage{#2}
            \fi
        \or % Figura posizionata nella prima pagina destra (numero dispari)
            \ifodd\value{page}
                \afterpage{#2}
            \else
                #2
            \fi
        \else % Figura posizionata senza preferenze
            #2
        \fi
        % Questo comando è pensato per figure flottanti che coprono un'intera pagina: difatti nel primo caso è un po' strano scrivere #2 se la pagina corrente è dispari quando si vuole l'elemento flottante in una pari; la risposta viene dal fatto che LaTeX non posiziona mai una figura flottante con posizione «p» nella pagina in cui la legge, ma la sposta almeno a quella seccessiva che è appunto pari. Un simile discorso vale anche per l'altro ramo condizionale
        % In ogni caso è un po' debole e dimostra che è difficile gestire tante figure flottanti posizionandole, per es., tutte a sinistra (pagine dispari), ma aiuta in ogni caso
    }
%\endgroup
...
%\begingroup Riferimenti
    ...
    \newcommand\crefsubfigInt[4][-]{Figg.~\ref{#2}(#3)#1\ref{#2}(#4)}
    \newcommand\crefsubfigSep[4]{Figg.~\ref{#1}(#2)\foreach \l in#3{,~\ref{#1}(\l)}~e~\ref{#1}(#4)}
    ...
    \crefname{prt}{Prop.}{Propp.}
    \crefrangeformat{prt}{Propp.~#3#1#4--#5#2#6}
    \crefmultiformat{prt}{Propp.~#2#1#3}{ e~#2#1#3}{, #2#1#3}{ e~#2#1#3}
    \crefrangemultiformat{prt}{Propp.~#3#1#4--#5#2#6}{ e~#3#1#4--#5#2#6}{, #3#1#4--#5#2#6}{ e~#3#1#4--#5#2#6}
%\endgroup
