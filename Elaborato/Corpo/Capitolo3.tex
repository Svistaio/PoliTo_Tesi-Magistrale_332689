...
\chapter[Teoria cinetica dei sistemi multiagente]{\texorpdfstring
    ...
    \section{Definizioni preliminari di probabilità}\label{secDefinizioniPreliminariProbabilitàTCSMA}
        ...
        Si annoverano ora alcuni concetti e risultati di teoria della probabilità avvisando, però, che per molti di essi è impossibile entrare eccessivamente nei dettagli; in ogni caso si rimanda a \cite{Durrett2019PTE} per gl'interessati.

        \begin{dfn}[Semirette]
            \label{defSemirettaInteraRealePositiva}
        ...
        \begin{dfn}[Variabile aleatoria]
            ...
            Una variabile aleatoria \(X\) è una funzione misurabile \(X\colon\Omega\to\mathbb R\), il cui dominio è uno spazio astratto \(\Omega\) di eventi elementari; quest'ultimo definisce a sua volta uno spazio di probabilità \((\Omega,\mathcal F,\mathbb P)\) composto da una \(\sigma\)-algebra \(\mathcal F\) degli eventi e una misura \(\mathbb P\) di massa unitaria.
        ...
        \begin{dfn}[Densità di probabilità di X]
            ...
            Sia \(X\in\mathbb R\) una variabile aleatoria assolutamente continua, allora la densità di probabilità \(f_X\colon\mathbb R\to\RPlus\) è una funzione misurabile che rappresenta la legge \(\mathbb P_X\):
            \[
        ...
        \begin{oss}
            Nella definizione precedente si assume che la variabile aleatoria \(X\) sia assolutamente continua, in tal modo \(f_X(x)\) è la sua derivata di Radon-Nikodym, ma la teoria sviluppata in questo paragrafo vale anche per misure piú astratte rispetto alle quali \(f_X(x)dx\) va inteso come \(f_X(dx)\).
        \end{oss}

        \begin{dfn}[Densità congiunta di probabilità di \(\mathbf X\)]
            \label{defDensitàCongiuntaProbabilità}
            Siano \(n\in\NPlus\) e \[\mathbf X=(X_1,X_2,\ldots,X_n)\in\mathbb R^n\] un vettore aleatorio assolutamente continuo, allora la densità congiunta di probabilità \(f_{\mathbf X}\colon\mathbb R^n\to\RPlus\) è una funzione misurabile che rappresenta la legge \(\mathbb P_{\mathbf X}\):
            \[
            ...
            \]
            ove \(\de\mathbf x\equiv\prod\de x_i=\de x_1\de x_2\cdots\de x_n\).
            % e soddisfà la condizione di normalità
            % \[
        ...
        \begin{dfn}[Densità marginale di probabilità di \(\mathbf X\)]
            ...
            Siano \(n\in\NPlus\) e \(\mathbf X\in\mathbb R^n\) un vettore aleatorio assolutamente continuo, allora la densità marginale di probabilità \(f_{X_i}\colon\mathbb R\to\RPlus\) è una funzione misurabile cosí definita:
            %
            \begin{equation}
        ...
        \begin{prt}[Densità congiunta di variabili aleatorie indipendenti]
            ...
            Siano \(n\in\NPlus\) e \(\mathbf X\in\mathbb R^n\) un vettore aleatorio assolutamente continuo, allora se tutte le variabili aleatorie che lo compongono sono tra loro indipendenti, la densità congiunta di \(\mathbf X\) equivale alla produttoria di quelle marginali:
            \[
                X_i\independent X_j
                \ \ \forall i\neq j\in\{1,2,\ldots,n\}
                \ \ \implies\ \ 
        ...
        \begin{dfn}[Valore atteso]
            ...
            \]
            e un simile discorso vale anche per un vettore aleatorio ma con \(\varphi\colon\mathbb R^n\to\mathbb R\), preso \(n\in\NPlus\).%:
            % \[
        ...
        \begin{dfn}[\(\sigma\)-algebra generata da \(X\)]
            Sia \(X\in\mathbb R\) una variabile aleatoria, essa induce una \(\sigma\)-algebra definita come
            \[
                \sigma(X)\equiv\sigma(\{X^{-1}(B)\ |\ B\in\mathcal B(\mathbb R)\}),
            \]
            ove \(\mathcal B(\mathbb R)\) è la \(\sigma\)-algebra di Borel relativa allo spazio dei numeri reali, mentre \(\sigma(\mathcal E)\) è la piú piccola \(\sigma\)-algebra che contiene \(\mathcal E\subset\mathcal F\).

            Un simile risultato vale anche per i vettori aleatori \(\mathbf X\in\mathbb R^n\).
        \end{dfn}

        \begin{dfn}[Valore atteso condizionato]
            \label{defValoreAttesoCondizionato}
            Sia \(X\in\mathbb R\) una variabile aleatoria a media finita \(\mathbb E[X]<\infty\) e sia \(\mathcal G\subseteq\mathcal F\) una sotto-\(\sigma\)-algebra di \(\mathcal F\), allora l'attesa di \(X\) condizionata a \(\mathcal G\) è una variabile aleatoria \(Y\in\mathbb R\) tale che
            ...
            L'insieme delle \(Y\) che soddisfanno le \ref{ipAttesaCondizionataGMisurabile} e \ref{ipAttesaCondizionataUgualeMisura} sono indicate col simbolo \(\mathbb E[X|\mathcal G]\) che ne è anche il suo rappresentante: una generica \(Y\) si può indicare anche direttamente come \(\mathbb E[X|\mathcal G]\).

            Un'analoga definizione vale anche per i vettori aleatori e per i casi misti in cui \(\mathbf X\in\mathbb R^n\) e \(\mathbf Y\in\mathbb R^m\), presi \(n,m\in\NPlus\).
        \end{dfn}

        %\begingroup Vecchia proposizione
            % \begin{prp}
        ...
        \begin{oss}
            Nel caso in cui \(\mathcal G\equiv\sigma(Y)\), data una variabile aleatoria \(Y\in\mathbb R\), si è soliti indicare l'attesa condizionata con \(\mathbb E[X|Y]\) sottintendendo \(\sigma(\cdot)\).
        \end{oss}

        \begin{oss}[Dalla teoria alla pratica]
            \label{ossAttesaCondizionataInterpretazionePratica}
            Esiste un'interpretazione piú pratica per l'attesa condizionata rispetto alla \cref{defValoreAttesoCondizionato} alquanto teorica: la \(\mathbb E[X|\mathcal G]\) si calcola mediando \(X\) \textit{considerando \(\mathcal G\) noto}; ciò implica che si sanno tutte le informazioni di \(\mathcal G\), ovverosia che i suoi eventi si sono realizzati.
           
            Un uso che permette di capire meglio quanto detto si può trovare nella proprietà \ref{ipAttesaCondizionataUgualeMisura} imponendo \(A=\Omega\), sempre lecito essendo \(\mathcal G\) una \(\sigma\)-al\-ge\-bra:
            %
            \begin{equation}
                \label{eqAttesaCondizionataInterpretazionePraticaGenerale}
                \mathbb E[X]
                =\mathbb E[\mathbb E[X|\mathcal G]],
            \end{equation}
            %
            che a parole significa che l'attesa di \(X\) si può calcolare mediando l'attesa condizionata; tuttavia \(\mathcal G\) continua a essere una generica \(\sigma\)-algebra, difficilmente rappresentabile, per cui è opportuno specializzare ulteriormente la \cref{eqAttesaCondizionataInterpretazionePraticaGenerale}.
            
            Dati \(n,h,k\in\mathbb N_+\) tali che \(n=h+k\), siano
            ...
            \begin{enumerate}[
                ...
                \item un vettore aleatorio \(\mathbf Z=(\mathbf X,\mathbf Y)\in\mathbb R^n\), con \(\mathbf X\in\mathbb R^h\) e \(\mathbf Y\in\mathbb R^k\), e% con densità congiunta \(f_{\mathbf Z}(\mathbf x,\mathbf y)\) e marginali \(f_{\mathbf X}\) e \(f_{\mathbf Y}\), e
                \item una funzione \(\varphi\colon\mathbb R^n\to\mathbb R\) misurabile e tale che \(\mathbb E[\varphi(\mathbf Z)]<\infty,\)%=\mathbb E[\varphi(\mathbf X,\mathbf Y)]
            \end{enumerate}

            allora, ipotizzando che \(\mathbf X(\mathbf Y)\) sia funzione di \(\mathbf Y\) e che quest'ultimo \textit{non} sia funzione\footnotemark{} di \(\mathbf X\), posto \(\mathcal G\equiv\sigma(\mathbf Y)\) la \cref{eqAttesaCondizionataInterpretazionePraticaGenerale} diventa%
            \footnotetext{Si noti che i due vettori \(\mathbf X\) e \(\mathbf Y\) sono per costruzione dipendenti.}%
            %
            \begin{equation*}
                \mathbb E[\varphi(\mathbf X(\mathbf Y),\mathbf Y)]
                =\mathbb E[\mathbb E[\varphi(\mathbf X(\mathbf Y),\mathbf Y)|\mathbf Y]];
            \end{equation*}
            %
            ma ora è facile capire cosa s'intende per \(\sigma(\mathbf Y)\) nota: si assuma che \(\mathbf Y\) abbia realizzato l'evento \(\mathbf y\) arbitrario\footnotemark{}, e, siccome non è funzione di \(\mathbf X\), ciò implica%
            \footnotetext{L'evento \(\mathbf y\) va visto come un punto se \(\mathbf Y\) è discreta, come un intervallo se è assolutamente continua e una via di mezzo se è mista.}%
            %
            \begin{equation}
                \label{eqAttesaCondizionataInterpretazionePraticaSpecializzata}
                \mathbb E[\varphi(\mathbf X(\mathbf Y),\mathbf Y)|\mathbf Y=\mathbf y]
                =\mathbb E[\varphi(\mathbf X(\mathbf y),\mathbf y)],
            \end{equation}
            %
            ...
            Lo stesso \textit{non} si può dire invertendo il condizionamento
            %
            \begin{equation*}
                \mathbb E[\varphi(\mathbf X(\mathbf Y),\mathbf Y)]
                =\mathbb E[\mathbb E[\varphi(\mathbf X(\mathbf Y),\mathbf Y)|\mathbf X]],
            \end{equation*}
            %
            sebbene l'identità rimanga valida; difatti in questo caso \(\mathbf X\) è funzione di \(\mathbf Y\): una sua realizzazione condiziona implicitamente \(\mathbf Y\) che non può essere di conseguenza arbitrario, per cui un'analoga relazione alla \cref{eqAttesaCondizionataInterpretazionePraticaSpecializzata}, in cui \(\mathbf X\) diventa questa volta un parametro, sarebbe in generale sbagliata.

            Da questo esempio è chiaro che considerare \(\mathcal G\) nota è a volte intuitivo, come nella \cref{eqAttesaCondizionataInterpretazionePraticaSpecializzata}, semplificando notevolmente il calcolo dell'attesa condizionata, e altre volte di meno nel qual caso non è banale esplicitare \(\mathbb E[X|\mathcal G]\).
        \end{oss}

        \begin{dfn}[Processo stocastico]
            \label{defProcessoStocastico}
            Dato \(n\in\mathbb\NPlus\), un processo stocastico è un insieme di vettori aleatori \(\mathbf X_t\in\mathbb R^n\) parametrizzati da un indice \(t\in I\subseteq\mathbb R\):
            \[
            ...
            \[
                ...
                \quad\forall t\in I\text{ e }\forall A\subseteq\mathbb R^n\text{ misurabile}.
            \]
    ...
    \section{Descrizione cinetica di [tipo] Boltzmann}\label{secDescrizioneCineticaClassicaETipoBoltzmann}
        ...
        \subsection{Equazione di Boltzmann omogenea}\label{secBoltzmannOmogeno}
            ...
            \subsubsection{Descrizione classica}
                ...
                \begin{ipo}
                    ...
                    \begin{enumerate}[
                        ...
                        \item\label{ipGasRarefatto} è rarefatto, da cui segue che solo le interazioni binarie possono aver luogo o, piú precisamente, sono piú frequenti;
                        \item\label{ipGasParticelleIndistinguibili} è composto da particelle indistinguibili e aventi ugual massa;
                        \item\label{ipGasCollisioniElastiche} le collisioni sono elastiche per cui si ha conservazione dell'impulso e dell'energia esprimibile, grazie alle \ref{ipGasRarefatto} e \ref{ipGasParticelleIndistinguibili}, come
                        %
                        \begin{align}
                            \mathbf v'+\mathbf v_*'
                            &=\mathbf v+\mathbf v_*,
                ...
                \begin{prp}[Regole d'interazione]\label{prpVelocitàCollisionali}
                    Esistono due funzioni \[\psi,\psi_*\colon\mathbb R^3\times\mathbb R^3\to\mathbb R^3\] bilineari che soddisfanno \cref{eqConservazioneImpulso,eqConservazioneEnergia} e tali che
                    %
                    \begin{equation}
                ...
                \begin{proof}
                    ...
                    \]
                    per cui \(\gamma\) è in realtà una funzione delle velocità precollisionali; in tal modo la \cref{eqAnsatzCollisioni} diventa la \cref{eqRegoleInterazioneDeterministicheGas}.
                    %
                    % \begin{equation}
                ...
                %
                \begin{oss}
                    Nella precedente dimostrazione \(n\in\mathbb S^2\) è lasciato arbitrario, seppure da un punto di vista fisico sia ragionevole porlo parallelo al vettore passante per i centri delle particelle collidenti.
                \end{oss}

                Le regole d'interazione nella \cref{eqRegoleInterazioneDeterministicheGas} sono anche simmetriche:
                ...
                \begin{dfn}
                    ...
                    \[
                        \begin{aligned}
                            \mathbf v'=\psi(\mathbf v,\mathbf v_*)&=\psi_*(\mathbf v_*,\mathbf v),\\
                            \mathbf v'_*=\psi_*(\mathbf v,\mathbf v_*)&=\psi(\mathbf v_*,\mathbf v),
                        \end{aligned}
                        % \ \ \implies\ \ 
                        % \begin{aligned}
                        ...
                        % \end{aligned}
                    \]
                    \(\forall\mathbf v,\mathbf v_*\in\mathbb R^3\), allora si dicono simmetriche.
                \end{dfn}
            ...
            \subsubsection{Descrizione statistica}
                ...
                \begin{oss}
                    ...
                    Le variabili aleatorie \(\mathbf V_t\) e \(\mathbf V^*_t\) non sono indicizzate (per es. \(\mathbf V^i_t\)) proprio per l'indistinguibilità delle particelle assunta dalla \ref{ipGasParticelleIndistinguibili}, da cui si evincono anche che le leggi di \(\mathbf V_t\) e \(\mathbf V^*_t\) sono identiche:
                    \[
                        f_{\mathbf V_t}(\mathbf v,t)=f_{\mathbf V^*_t}(\mathbf v,t)=f(\mathbf v,t),
                        \quad\forall \mathbf v\in\mathbb R^3.
                    \]
                \end{oss}
                ...
                \eqtag*{\([\text{RI}]_{\mathbf V}\)}{
                    ...
                    [\text{RI}]_{\mathbf V}
                    \left\{\begin{aligned}
                        \mathbf V_t'&=\psi(\mathbf V_t,\mathbf V_t^*)\equiv\mathbf V_t+[(\mathbf V_t^*-\mathbf V_t)\cdot n]n,\\
                        \mathbf V_t^{*\prime}&=\psi^*(\mathbf V_t,\mathbf V_t^*)\equiv \mathbf V_t^*+[(\mathbf V_t-\mathbf V_t^*)\cdot n]n,
                    \end{aligned}\right.
                }%
                in cui anche \(n\) risulta aleatoriamente distribuito; usualmente si postula che non vi siano direzioni preferenziali, ossia \(n\sim\mathcal U(\mathbb S^2)\), essendo queste uniformemente distribuite sulla sfera unitaria.
                ...
                che è una variabile aleatoria di Bernoulli, dipendente da \(\mathbf V_t\) e \(\mathbf V^*_t\), la cui probabilità è descritta da due termini:
                ...
                \begin{enumerate}[
                    ...
                    \item la funzione \(B\colon\mathbb R\to\RPlus\), detta nucleo di collisione, che permette d'avere una maggiore espressività sulle collisioni molecolari\footnotemark{}, e%
            ...
            \subsubsection{Derivazione modello}
                ...
                L'idea successiva, al fine di ricavare un modello dell'evoluzione per la densità \(f\), è di mediare le \ref{eqAlgoritmoInterazioniGas} attraverso una funzione arbitraria \(\varphi\colon\mathbb R^3\to\mathbb R\), detta quantità osservabile, computabile dalle realizzazioni o di \(\mathbf V_{t+\Delta t}\) o di \(\mathbf V^*_{t+\Delta t}\); pertanto applicando \(\varphi\) alle \ref{eqAlgoritmoInterazioniGas},
                %
                \begin{equation*}
                    \begin{aligned}
                ...
                \begin{equation}
                    ...
                    \begin{aligned}
                        ...
                        \\&\quad+\mathbb E[\varphi(\mathbf V'_t)B((\mathbf V^*_t-\mathbf V_t)\cdot n)]\Delta t,
                    \end{aligned}
                \end{equation}
                %
                ...
                \end{equation*}
                %
                e passando formalmente al tempo continuo col limite\footnotemark{} \(\Delta t\to0^+\) si ha
                \footnotetext{Ciò permette di soddisfare sempre la condizione nell'\cref{ossCondizioneTetaGas}.}
                %
                \begin{equation*}
                    \begin{aligned}
                ...
                \begin{equation}
                    ...
                    \newcommand\hsepI{\hspace{-.7em}}
                    \newcommand\hsepII{\hspace{-.5em}}
                    \begin{aligned}
                        \derS{}t\int_{\mathbb R^3}\hsepI
                        \varphi(\mathbf v)f(\mathbf v,t)\de\mathbf v
                        &=\int_{\mathbb R^6}\hsepII\langle
                            B(\mathbf v_*,\mathbf v,n)
                ...
                \end{equation}
                %
                in cui \(B(\mathbf v_*,\mathbf v,n)\) è una forma breve per \(B((\mathbf v_*-\mathbf v)\cdot n)\), 
                \(
                    \langle\cdot\rangle\equiv
                    \frac1{4\pi}\int_{\mathbb S^2}
                    \cdot\de n
                \) 
                indica la media rispetto a \(n\), e si è posto \(\mathbf V\equiv(\mathbf V_t,\mathbf V^*_t)\) con densità congiunta \(f_{\mathbf V}(\mathbf v,\mathbf v_*,t)\). È infine necessario sommare le \eqref{eqInterazioniOsservabiliContinue}; s'inizi dal primo membro:
                %
                \begin{equation}
                ...
                \begin{ipo}
                    ...
                    \begin{equation}
                        ...
                        B(\mathbf v_*,\mathbf v,n)=B((\mathbf v_*-\mathbf v)\cdot n)=
                        B((\mathbf v-\mathbf v_*)\cdot n)=B(\mathbf v,\mathbf v_*,n),
                        \quad\forall\mathbf v,\mathbf v_*\in\mathbb R^3;
                    \end{equation}
                    %
                    una scelta tipica è il valore assoluto \(\vert\cdot\vert\): \(B(\mathbf v_*,\mathbf v,n)\equiv\vert(\mathbf v_*-\mathbf v)\cdot n\vert\).
                \end{ipo}

                \begin{ipo}[Caos molecolare]
                    \label{ipCaoMolecolare}
                    Le particelle interagenti secondo l'\ref{eqAlgoritmoInterazioniGas} sono campionante indipendentemente. Tal'ipotesi è piú facile da giustificare matematicamente che fisicamente perché semplifica di molto i conti; in ogni caso, la \ref{ipGasRarefatto} la corrobora poiché in un gas rarefatto è naturale che se due particelle interagiscono, prima che ricollidano, avranno perso ogni vicendevole dipendenza a causa delle innumerevoli altre interazioni colle altre particelle.
                \end{ipo}

                \begin{oss}
                    Dal'\cref{ipCaoMolecolare}, unitamente alla \cref{prtDensitàCongiuntaIndipendente}, si evince che la densità congiunta del vettore aleatorio \(\mathbf V\equiv(\mathbf V_t,\mathbf V^*_t)\) è data dal prodotto
                    \[
                        f_{\mathbf V}(\mathbf v,\mathbf v_*,t)
                        =f_{\mathbf V_t}(\mathbf v,t)f_{\mathbf V^*_t}(\mathbf v_*,t)
                        % \overset{\text{\ref{ossLeggiIndistinguibili}}}
                        =f(\mathbf v,t)f(\mathbf v_*,t),
                        \quad\forall \mathbf v,\mathbf v_*\in\mathbb R^3.
                    \]
                \end{oss}

                In tal modo il termine moltiplicato a \(\varphi(\mathbf v_*)\) nel secondo membro della seconda equazione della \eqref{eqInterazioniOsservabiliContinue} può essere cosí riformulato:
                %
                \begin{equation}
                ...
                \end{equation}
                %
                cambiando le variabili analogamente alla \eqref{eqSommaPrimoMembroIOC}. Applicando i due risultati illustrati nelle \cref{eqSommaPrimoMembroIOC,eqSommaSecondoMembroIOC} si perviene finalmente all'equazione di Boltzmann omogenea asimmetrica in forma debole:
                %
                \begin{equation}
                    ...
                    \frac1{4\pi}\int_{\mathbb R^6}\int_{\mathbb S^2}
                        % \left\langle % La media di n è già esplicita
                ...
                \end{equation}
                %
                ove, per brevità di notazione, gli apici \(\prime\) e asterischi \(*\) sottintendono gli argomenti\footnotemark{} per tutte le funzioni, eccetto per il nucleo di collisione.
                \footnotetext{Per una spiegazione piú dettagliata si legga poco dopo nel \cref{secTipoBoltzmannOmogeno}.}
                ...
                %
                \begin{equation*}
                    \int_{\mathbb R^6}\left\langle
                        B(\mathbf v,\mathbf v_*,n)\varphi'
                    \right\rangle ff_*\de\mathbf v\de\mathbf v_*,
                    =\int_{\mathbb R^6}\left\langle
                        B(\mathbf v,\mathbf v_*,n)\varphi'_*
                    \right\rangle ff_*\de\mathbf v\de\mathbf v_*,
                \end{equation*}
                %
                da cui segue l'equazione di Boltzmann omogenea simmetrica in forma debole\footnotemark{}:%
                \footnotetext{Si dice debole siccome essa vale per una funzione \textit{test} \(\varphi\) arbitraria.}%
                %
                \eqtag{\([\text{EB}]_{\mathbf V}\)}{
                ...
                Si può anche ricavare la forma forte della \ref{eqFormaDeboleBoltzmannOmogeneoSimmetrico} considerando le regole d'interazione inverse delle \ref{eqRegoleInterazioneGas} \cite[§ 2.5, p. 15]{Loy2025EKTMAS}, ma il conto esula dagli scopi di questo elaborato; cionnonostante si riporta come riferimento per il paragrafo a venire:
                %
                \begin{equation}
        ...
        \subsection{Equazione di Boltzmann disomogenea}\label{secBoltzmannDisomogeno}
            ...
            Piú nel dettaglio, il primo membro non è altro che la derivata materiale di \(f\): difatti, posto \(B(\mathbf v,\mathbf v_*,t)\equiv0\), ovvero in mancanza di collisioni, l'equazione involve in una del trasporto lineare con soluzione nota.
            ...
            \begin{proof}
                Si procede usando il metodo delle caratteristiche: nella \eqref{eqTrasportoLineare} la velocità \(\mathbf v\) nell'operatore avvettivo non dipende né dallo spazio né dal tempo, quindi si possono prendere le curve, dette appunto caratteristiche, \(\hat{\mathbf x}(t)\) tali che
                %
                \begin{equation}
                    \label{eqCurveCaratteristiche}
                    \derS{\hat{\mathbf x}}t=\mathbf v
                    \ \ \implies\ \ 
                    \hat{\mathbf x}(t)=\hat{\mathbf x}_0+\mathbf vt
                \end{equation}
                %
                ...
                \[
                    \derS{\hat f}t
                    =\derP ft+\derS{\mathbf x}t\cdot\nabla f
                    =\derP ft+\mathbf v\cdot\nabla f=0,
                \]
                perciò \(\hat f(t)\) è costante \(\forall t\geq0\), e ciò vale per ogni caratteristica essendo \(\hat{\mathbf x}_0\) arbitrario; eppure, preso un punto generico \((\tilde{\mathbf x},\mathbf v,\tilde t)\) dello spazio delle fasi, è possibile definire il piede \(\hat{\mathbf x}_0\equiv\tilde{\mathbf x}-\mathbf v\tilde t\) da cui
                \[
                    \hat{\mathbf x}(\tilde t)
                    =\hat{\mathbf x}_0+\mathbf v\tilde t
                    =\tilde{\mathbf x},
                \]
                che porta rapidamente alla soluzione trascurando la tilde:
                \[
                    f(\mathbf x,\mathbf v,t)
                    =f(\hat{\mathbf x}(t),\mathbf v,t)
                    =\hat f(t)=\hat f(0)
                    =f(\hat{\mathbf x}_0,\mathbf v,0)
                    =f_0(\hat{\mathbf x}_0,\mathbf v)
            ...
            D'altra parte il secondo membro della \eqref{eqFormaForteBoltzmannDisomogeneoSimmetrico} rappresenta la variazione media statistica dalle velocità molecolari a causa delle collisioni, e viene perciò anche chiamato operatore collisionale.
            ...
            Ciò è stato possibile introducendo i momenti statistici, ossia quantità macroscopiche calcolabili a partire dalla distribuzione \(f\), soluzione della \eqref{eqFormaForteBoltzmannDisomogeneoSimmetrico}; i principali sono, sottintendendo gli argomenti come nella \ref{eqFormaDeboleBoltzmannOmogeneoSimmetrico},
            %
            \begin{equation}
                \label{eqPrincipaliMomentiBoltzmannGas}
                \def\hsep{\hspace{1em}}
                \begin{aligned}
                    \rho(\mathbf x,t)&\equiv\int_{\mathbb R^3}f\de\mathbf v,
                    \hsep&E(\mathbf x,t)&\equiv\frac1\rho\int_{\mathbb R^3}\Vert\mathbf v\Vert^2f\de\mathbf v,\\
                    \mathbf u(\mathbf x,t)&\equiv\frac1\rho\int_{\mathbb R^3}\mathbf vf\de\mathbf v,
                    \hsep&e(\mathbf x,t)&\equiv\frac1\rho\int_{\mathbb R^3}
                    \Vert\mathbf v-\mathbf u\Vert^2f\de\mathbf v,\\
                \end{aligned}
            \end{equation}
            % \begin{equation}
            %     \label{eqPrincipaliMomentiBoltzmannGas}
            ...
            % \end{equation}
            %
            rispettivamente densità, energia totale, velocità massica ed energia interna\footnotemark{}. Tali quantità sono definite a partire dalla \(f\), seppure questa sia in genere infattibile da ricavare dalla \eqref{eqFormaForteBoltzmannDisomogeneoSimmetrico}, vista la difficile trattabilità analitica dell'equazione; eppure, tramite una sua attenta riformulazione in determinati regimi limite \cite{Loy2025EKTMAS,PareschiIMS2013}, è possibile svincolarsi del tutto dall'evoluzione puntuale della \(f\) ricavando un sistema chiuso dei soli momenti \eqref{eqPrincipaliMomentiBoltzmannGas}. La convergenza di questi a un valore stazionario è il nesso tra i due mondi attraverso una descrizione mesoscopica indotta dalla distribuzione \(f\).%
            \footnotetext{Un'altra quantità macroscopica di rilievo legata a \(e\) è la temperatura \(\theta(\mathbf x,t)\equiv\frac13 e(\mathbf x,t)\).}%

        \subsection{Equazione di tipo Boltzmann omogenea}\label{secTipoBoltzmannOmogeno}

            La derivazione dettagliata nel \cref{secBoltzmannOmogeno} è valida in realtà per una classe di problemi molto piú generali, detti \textit{di tipo} Boltzmann omogenei, la cui teoria è stata sviluppata da Pareschi \etal{} \cite{PareschiIMS2013}; sono chiamati cosí per due ragioni:
            ...
            \begin{enumerate}[
                ...
                \item sono applicati a contesti\footnotemark{} molto distanti da quello originale di una popolazione di particelle di un gas.%
                \footnotetext{Esempi importati sono quello sociofisico, come i vari modelli sulle opinioni, ed econofisico, come quello qui considerato delle città, sebbene contenga elementi anche del primo.}%
            \end{enumerate}
            
            Dunque in questo paragrafo si generalizzano molti risultati analoghi a quelli già visti nel \cref{secBoltzmannOmogeno} senza però riscrivere tutt'i passaggi.

            \subsubsection{Descrizione e derivazione}

                Sia \(\{\mathbf X_t\}_{t\in\RPlus}\) il processo stocastico relativo allo stato microscopico dell'agente di un sistema, dove \(\mathbf X_t\in I\subseteq\mathbb R^n\) è il vettore aleatorio\footnotemark{} che descrive i suoi \(n\in\mathbb N_+\) stati microscopici al tempo \(t\); allora le \cref{ipGasPerfetto,ipCaoMolecolare} diventano:%
                \footnotetext{Si noti che il dominio \(I\) degli stati non coincide necessariamente coll'intero spazio reale \(\mathbb R^n\).}%

                \begin{ipo}
                    \label{ipTipoBoltzmann}
                    ...
                    \begin{enumerate}[
                        ...
                        \item\label{ipTipoBoltzmannDensitàOmogenea} la distribuzione statistica dei microstati è uniforme nello spazio;
                        \item\label{ipTipoBoltzmannInterazioniBinarie} le interazioni non binarie tra agenti sono trascurabili;
                        \item\label{ipTipoBoltzmannAgentiIndistinguibili} gli agenti sono indistinguibili e ciascuno ne rappresenta l'insieme;
                        \item\label{ipTipoBoltzmannCaoMolecolare} due agenti che interagiscono sono statisticamente indipendenti.
                    \end{enumerate}
                    %
                ...
                \eqtag{\([\text{NI}]_{\mathbf X}\)}{
                    ...
                    \mu\colon\mathbb R^n\times\mathbb R^n
                    \times\mathbb R^k\times\RPlus\to\RPlus,
                }%
                ...
                \begin{oss}
                    Se non si sa o non si vuole toccare quest'aspetto modellistico si può semplicemente porre unitario:
                    \[
                        \mu\equiv1,\quad\forall(\mathbf x,\mathbf x_*,\mathbf w,t)
                        \in\mathbb R^n\times\mathbb R^n\times\mathbb R^k\times\RPlus
                    \]
                ...
                \begin{oss}
                    ...
                    La sigla \ref{eqAlgoritmoInterazione} sta per «Azione-Reazione» (\cref{figRappresentazioneGraficaAzioneReazione}) poiché si suppone che ogn'interazione (azione), ossia \(\Theta=1\), necessariamente modifica entrambi gli stati degli agenti coinvolti (reazione); in alcuni contesti \cite[(2.5) p. 5]{Nurisso2024} questa regola può essere alterata dimodoché solo l'agente che interagisce sia modificato portando alle interazioni «Azione-Azione».
                \end{oss}

                % \tikzfigure{htb}
                %     {\input{../../Figure/.fnl/Capitolo3/RappresentazioneGraficaAzioneReazione.tex}}
                \tikzexternalizedfigure{htb}
                    {../../Figure/.fnl/Capitolo3/RappresentazioneGraficaAzioneReazione.tex}
                    {Rappresentazione grafica dell'\ref{eqAlgoritmoInterazione}.}
                    {.8\linewidth}{figRappresentazioneGraficaAzioneReazione}

                Infine, riapplicando un ragionamento analogo a quello svolto nel \cref{secBoltzmannOmogeno} colla quantità osservabile \(\varphi\colon\mathbb R^n\to\mathbb R\) per ricavare la \ref{eqFormaDeboleBoltzmannOmogeneoSimmetrico}, si perviene all'equazione di tipo Boltzmann omogenea
                %
                \eqtag{\([\text{EtB}]_{\mathbf X}\)}{
                    ...
                    \int_{\mathbb R^{2n}}\left\langle
                        \mu(\mathbf x,\mathbf x_*,\mathbf w,t)
                ...
                \begin{dfn}[\ref{eqNucleoInterazione} pari]
                    ...
                    Se il tasso d'interazione verifica
                    \[
                    ...
                    \]
                    allora \(\mu\) è detto pari.
                \end{dfn}

                \begin{dfn}[\ref{eqRegoleInterazione} simmetriche]
                    \label{defLeggiInterazioneSimmetriche}
                    Se le regole d'interazione verificano
                    \[
                        \begin{aligned}
                            \mathbf x'_t=\psi(\mathbf x,\mathbf x^*,\mathbf y)&=\psi_*(\mathbf x^*,\mathbf x,\mathbf y),\\
                            \mathbf x^{*\prime}_t=\psi_*(\mathbf x,\mathbf x^*,\mathbf y_*)&=\psi(\mathbf x^*,\mathbf x,\mathbf y_*),
                        \end{aligned}
                        \quad
                    \]
                    \(\forall\mathbf x,\mathbf x_*\in I\subseteq\mathbb R^n\) e \(\forall\mathbf y,\mathbf y_*\in\mathbb R^h\), allora si dicono simmetriche.% In altre parole, la simmetria implica
                    % \[
                ...
                \begin{oss}[\ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} simmetrica]
                    Qualora l'\ref{eqNucleoInterazione} sia pari e le \ref{eqRegoleInterazione} siano simmetriche, la \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} diventa
                    %
                    \begin{equation}
                    ...
                    \end{equation}
                    %
                    con un cambio di variabili per invertire \(\mathbf x\) e \(\mathbf x_*\) nella differenza \(\varphi'_*-\varphi_*\).
                \end{oss}

            \subsubsection{Notazione breve}

                Nei previ paragrafi si è fatto spesso uso, specie per le varie equazioni [di tipo] Boltzmann \cref{eqFormaDeboleBoltzmannOmogeneoAssimmetricoGas,eqFormaForteBoltzmannDisomogeneoSimmetrico,eqFormaForteBoltzmannOmogeneoSimmetrico}, \ref{eqFormaDeboleBoltzmannOmogeneoSimmetrico} e \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico}, di una notazione abbreviata per tutti gli oggetti salvo il tasso d'interazione \(\mu\).
               
                Si definiscono adesso esplicitamente queste abbreviazioni facendo riferimento al contesto generale appena sviluppato. Innanzitutto la densità congiunta \(f\) da sola sottintende la variabile dell'agente interagente \(\mathbf x\) mentre con un pedice \(f_*\) quella dell'agente ricevente \(\mathbf x_*\):
                %
                ...
                \end{equation*}
                %
                dopodiché le quantità osservabili \(\varphi\) seguono la medesima logica di prima ma coll'aggiunta di un apice per distinguere i postati:
                %
                \begin{equation*}
                    \newcommand\hspc{\hspace{\valtmp}}
                    \begin{aligned}
                ...
                \begin{equation*}
                    \newcommand\hspc{\hspace{\valtmp}}
                    f_i\equiv f_i(\mathbf x,t)
                    \hspc\text{e}\hspc
                    f_i^*\equiv f_i(\mathbf x_*,t).
                \end{equation*}

            \subsubsection{Analisi dimensionale}

                Tutte le equazioni finora analizzate fanno riferimento a un tempo adimensionale, sebbene l'introduzione di una dimensione sia una modifica alquanto indolore e lasci trasparire un'interessante interpretazione di \(\Delta t\).
                
                Sia dunque \(\tilde t=\tau t\) il tempo dimensionale scomposto nel prodotto della dimensione \(\tau\) col tempo adimensionale \(t\).
                ...
                \end{equation}
                %
                Per capire come varia il tasso d'interazione \(\mu\) è sufficiente dimensionalizzare la \ref{eqBernoulliInterazione}:
                %
                \begin{equation*}
                ...
                \begin{oss}[frequenza d'interazione]
                    Da \(\Delta\tilde t=\tau\Delta t\) si può interpretare \(\Delta t\) come la frazione della dimensione \(\tau\) dopo la quale può avvenire un'interazione, da cui si evince [banalmente] che \(\Delta\tilde t\) è l'intervallo di tempo tra due interazioni. Di conseguenza si può vedere l'\ref{eqAlgoritmoInterazione} come un fenomeno periodico di periodo \(\Delta\tilde t\) e frequenza
                    %
                    \begin{equation*}
                        \tilde\nu\equiv\frac1{\Delta\tilde t}=\frac1{\tau\Delta t}\equiv\frac1\tau\nu,
                    \end{equation*}
                    %
                    in cui, come per il tasso d'interazione, \(1/\tau\) è la dimensione della frequenza adimensionale \(\nu\). Ecco che il reciproco del passo temporale \(\Delta t\) prende il significato della frequenza d'interazione del fenomeno, nell'unità di tempo sia adimensionale che dimensionale mediante \(1/\tau\).%
                \end{oss}

                \begin{oss}[Scelta di \(\tau\)]
                    \label{ossSceltaScalaTemporale}
                    ...
                    \begin{itemize}[
                        ...
                        \item quella dei mesi o superiori, perché è irrealistico che in un arco temporale cosí lungo vi siano solo 100 interazioni tra le città, e
                        \item parimenti quella dei secondi o inferiori per un ragionamento simile a quello di prima ma opposto.
                    \end{itemize}
                ...
                \begin{enumerate}[
                    ...
                    \item mancano i dati storici, sia attendibili che su finestre temporali adeguate, per poter confrontare il transitorio simulato con quello reale.
                \end{enumerate}
    ...
    \section{Descrizione cinetica urbana su reti}\label{secDescrizioneCineticaUrbana} %  d'interazioni mediante da un grafo
        ...
        Si affronta adesso il caso del tessuto interurbano descritto nel \cref{secRetiECittà}, considerando le città sia come agenti che come nodi di una rete che ne regola le interazioni: due città possono interagire sse sono connesse; è anche ovvio che l'interazione modifichi, in qualche modo da definire, la loro popolazione.

        A grandi linee in questo paragrafo si applicano i concetti della teoria sviluppata nei \cref{secDefinizioniGrafi,secTipoBoltzmannOmogeno} e, in particolare, la notazione definita. S'inizia mostrando una derivazione tramite la topologia esatta; quindi si approfondisce una variante nella quale si rilassa la topologia tramite una sua approssimazione; infine, si studiano i collegamenti tra la derivazione esatta e quella di tipo Boltzmann sotto determinate ipotesi.

        \subsection{Equazione di tipo Boltzmann esatta}\label{secTipoBoltzmannUrbano}
            
            \subsubsection{Descrizione e derivazione}

                Visto che si assume che valgono le \cref{ipTipoBoltzmann}\footnotemark{}, dalla \ref{ipTipoBoltzmannAgentiIndistinguibili} sorge un problema non di poco conto: gli agenti sono indistinguibili ma la rete sottostante li rende distinti per via degl'indici \(\mathcal I\). Per dipanare la questione è sufficiente considerare l'indice come una variabile aleatoria \(I\in\mathcal I\) cosí da definire il processo stocastico come%
                \footnotetext{Perdipiú la \ref{ipTipoBoltzmannInterazioniBinarie} è valida \textit{per un passo} \(\Delta t\), ma nell'arco temporale \(\tau\) le interazioni si possono considerate anche come non binarie, come accennato nell'\cref{ossSceltaScalaTemporale}.}%
                %
                \begin{equation*}
                ...
                % \tikzfigure{htb}
                %     {\input{../../Figure/.fnl/Capitolo3/RappresentazioneGraficaTCSMAReti.tex}}
                \tikzexternalizedfigure{htb}
                    {../../Figure/.fnl/Capitolo3/RappresentazioneGraficaTCSMAReti.tex}
                    {Rappresentazione grafica della \protect\TCSMA{} su reti.}
                    {\linewidth}{figRappresentazioneGraficaTCSMAReti}

                \begin{oss}[Sulla natura numerica della popolazione]
                    \label{ossNaturaNumericaPopolazione}
                    È naturale che non esistano frazioni di persone e che quindi rigorosamente \(S_t\in\mathbb N\) ma non sempre la scelta piú realistica è piú modellisticamente agevole; infatti trattare \(S_t\) come una variabile aleatoria discreta impone un codominio, appunto, discreto che è in genere piú difficile da manipolare matematicamente in confronto a un intervallo continuo. In ogni caso, ammesso sia necessario, si può poi stimare il numero di abitanti effettivo approssimando i risultati per difetto; l'errore cosí commesso è di al piú una persona.
                \end{oss}
                ...
                %
                \begin{equation*}
                    f_i:\RPlus\times\RPlus\rightarrow\RPlus
                \end{equation*}
                %
                è la densità di \(S_t\) relativa all'\(i\)-esimo agente (\cref{figRappresentazioneGraficaTCSMAReti}).
                ...
                \eqtag{\([\text{Ber}]^{\mathbf A}_S\)}{
                    ...
                    \Theta\sim\DstrBernoulli(A(I,I_*)\Delta t),
                }%
                ove \(A\colon\mathcal I\times\mathcal I\to\mathbb R\) è una funzione che a ogni coppia d'indici \((I,I_*)\) associa il relativo elemento della matrice d'adiacenza
                %
                \begin{equation*}
                    \newcommand\hspz{\hspace{1em}}
                    A(I,I_*)\equiv a_{I,I_*}=
                    \left\{\begin{aligned}
                        1&\hspz\text{se }(I,I_*)\in\mathcal E,\\
                        0&\hspz\text{ altrimenti},%(I,I_*)\notin\mathcal E,
                    \end{aligned}\right.
                \end{equation*}
                %
                secondo la \cref{eqMatriceAdiacenzaUnitaria}; cosí definita la \ref{eqBernoulliInterazioneUrbana} inibisce \textit{a priori} l'interazione se i due agenti non sono connessi.
                ...
                \begin{oss}
                    ...
                    \begin{equation}
                        ...
                        % \mu(\mathbf X_t,\mathbf X^*_t,\mathbf w,t)\equiv A(I,I_*),
                ...
                D'altro canto le \ref{eqRegoleInterazione} diventano
                %
                \eqtag*{\([\text{RI}]_S\)}{
                ...
                \begin{oss}
                    Confrontate alle \ref{eqRegoleInterazione}, solo la prima funzione della città interagente \(\psi\) presenta un coefficiente stocastico \(y\equiv\gamma\in\mathbb R\), mentre quella relativa alla città ricevente \(\psi_*\) non dipende da potenziali coefficienti\footnotemark{} \(\mathbf y_*\).%
                ...
                Unendo la \ref{eqBernoulliInterazioneUrbana} e le \ref{eqRegoleInterazioneUrbane}, l'\ref{eqAlgoritmoInterazione} si scrive
                %
                \eqtag*{\([\text{AR}]_S\)}{
                ...
                coerentemente col fatto che in un contesto urbano, qualora una città-nodo interagisca con un'altra, entrambe devono variare il loro stato (\cref{ossAlgoritmoAzioneReazione}).

                \begin{oss}
                    Paragonato all'\ref{eqAlgoritmoInterazione} ci si potrebbe chiedere perché non si considera l'intero vettore aleatorio \(\mathbf X_t\), come pure nelle \ref{eqRegoleInterazioneUrbane}: la ragione è che \(I\) è una componente statica che non varia nel tempo.
                
                    Volendo però includere anche quelle componenti, ossia considerando \(I_t\) e \(I^*_t\) momentaneamente dinamiche, si possono definire le regole d'interazione
                    \[
                        [\text{RI}]_I\left\{
                        \begin{aligned}
                    ...
                    \[
                        ...
                        \end{aligned}\right.
                    \]
                    che soddisfà la staticità di \(I\) e \(I_*\) e completa, assieme all'\ref{eqAlgoritmoInterazioneUrbano}, la formulazione dell'\ref{eqAlgoritmoInterazione} piú generale.
                \end{oss}

                Sia ora \(\Phi\colon\mathcal I\times\RPlus\to\mathbb R\) un'arbitraria quantità osservabile, dalla \ref{eqBernoulliInterazioneUrbana} l'equazione di tipo Boltzmann omogenea \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} ha forma
                %
                ...
                \begin{oss}
                    Anche se la derivazione della \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetricoUrbano} è già stata spiegata nel dettaglio nel \cref{secBoltzmannOmogeno}, rimane stimolante riportare il passaggio piú difficile, ossia la media condizionata \cref{eqMediaCondizionataGas}:
                    %
                    \begin{equation*}
                        %\label{}
                        \begin{aligned}
            ...
            \subsubsection{Analisi delle regole d'interazione}
                ...
                Innanzitutto, passando alle realizzazioni di tutte le variabili aleatorie considerate, vale a dire scrivendole in minuscolo, e omettendo la dipendenza dal tempo, le \ref{eqRegoleInterazioneUrbane} sono cosí definite\footnotemark{}%
                ...
                \begin{equation*}
                    % \begin{aligned}
                        E,I\colon\RPlus\times\mathcal I\times\RPlus\times\mathcal I\to\RPlus,%\\
                        % I\colon\RPlus\times\mathcal I\times\RPlus\times\mathcal I\to\RPlus,
                    % \end{aligned}
                \end{equation*}
                %
                sono rispettivamente i tassi d'emigrazione e d'immigrazione, i cui argomenti sono per brevità sottintesi da qui in poi, mentre \(\gamma\) rappresenta fluttuazioni stocastiche. Si caratterizzano ulteriormente \(E\) e \(\gamma\):

                \begin{ipo}[Vincoli di positività]
                    \label{ipLimitiTassoEmigrazione}
                    Se \(\gamma=0\), per avere un postato \(s\) fisicamente sensato, ossia non negativo, si assume che \(E\) sia superiormente limitato dalla costante \(\lambda\equiv\sup E\in(0,1)\), e inferiormente limitato dall'origine:
                    %
                    \begin{equation*}
                        0\leq E\leq\lambda<1,\ \ \forall s,\ \forall s_*,\ \forall i,\ \forall i_*.
                    \end{equation*}
                    %
                    D'altra parte il postato \(s_*'\) non si considera poiché è per definizione sempre positivo qualunque sia il tasso d'immigrazione \(I\in\RPlus\).
                    
                    Invece se \(\gamma\neq0\), la perturbazione va limitata inferiormente:
                    %
                    \begin{equation}
                ...
                La scelta di \(>\) anziché \(\geq\) nella \eqref{eqVincoloPositivitàFluttuazioni} è ben fondata: di fatto si sta supponendo che le fluttuazioni non possano spopolare una città; tale condizione è necessaria perché nei tassi d'emigrazione venturi, proposti nel \cref{secRegoleEmigrazione}, compare il rapporto \(s_*/s\) o, equivalentemente, \(s/s_*\). Ciò non significa che il modello non possa prevedere un tale fenomeno, poiché la taglia può arbitrariamente avvicinarsi a zero, ma semplicemente che una città non potrà mai avere popolazione nulla colle \ref{eqRegoleInterazioneUrbaneEsplicite}; il valore nullo è raggiunto solo \textit{a posteriori} dopo l'approssimazione dai numeri reali a quelli naturali (\cref{ossNaturaNumericaPopolazione}).

                Passando alle fluttuazioni \(\gamma\), esse rappresentano a grandi linee quei fenomeni complessivi di nascita e di morte che vengono considerati ma non direttamente modellati; pertanto valgono le seguenti ipotesi:

                \begin{ipo}
                    \label{ipFluttuazioni}
                    La variabile aleatoria \(\gamma\) deve soddisfare tre caratteristiche:
                    ...
                    \begin{enumerate}[
                        ...
                        \item\label{ipFluttuazioniPositivitàNegatività} può assumere sia valori positivi che negativi, ma non minori del vincolo imposto dalla \eqref{eqVincoloPositivitàFluttuazioni};
                        \item\label{ipFluttuazioniMediaNulla} la media è scelta arbitrariamente posta a zero, ossia \(\langle\gamma\rangle=0\);
                ...
                \begin{enumerate}[
                    ...
                    \item l'unica distribuzione che può soddisfare tutt'e tre le caratteristiche ricercate è proprio la distribuzione \(\DstrGamma(\alpha,\beta)\).
                \end{enumerate}
                ...
                \end{equation*}
                %
                dove \(\alpha\) e \(\beta\) sono i parametri ordinatamente di forma e di scala, mentre
                %
                \begin{equation*}
                ...
                \end{equation*}
                %
                Prima di tutto la \ref{ipFluttuazioniMediaNulla} si può soddisfare semplicemente traslando i valori campionanti di una qualunque \(\hat\gamma\) di \(-\langle\hat\gamma\rangle\), ossia si considera la distribuzione \(\gamma\sim\hat\gamma-\langle\hat\gamma\rangle\):
                %
                \begin{equation*}
                    \langle\gamma\rangle=
                ...
                %
                \begin{subequations}
                    \newcommand\hsep{\hspace{2em}}
                    \begin{alignat*}{3}
                        \langle\hat\gamma\rangle&\equiv1-E=\alpha\beta
                        &\hsep\text{e}\hsep&
                        &\Varianza(\hat\gamma)&\equiv\sigma^2=\alpha\beta^2,\\
                \intertext{in cui \(\sigma\) indica la deviazione, da cui}
                ...
                \end{subequations}
                %
                cosí da soddisfare \ref{ipFluttuazioniPositivitàNegatività} una volta applicata la traslazione \(-\langle\hat\gamma\rangle\).
               
                Tuttavia bisogna salvaguardarsi dai casi degeneri della distribuzione gamma: essa infatti se \(\alpha=1\) diventa \(\DstrEsponenziale(\beta)\), e se \(\alpha<1\) diverge all'origine; per avere quindi densità nulla all'origine è necessario porre
                %
                \begin{equation*}
                ...
                \end{equation*}
                %
                che implica come non sia possibile avere una deviazione arbitraria per poter soddisfare la \ref{ipFluttuazioniPositivitàNegatività}, ma che questa è limitata superiormente dal massimo tasso d'emigrazione \(\lambda\): piú grande è \(\lambda\) piú piccola è \(\sigma\), e viceversa.

                In realtà la \(\sigma\) non può davvero assumere valori arbitrari tra \(0\) e \(1-\lambda\): difatti se \(\sigma\to1-\lambda\) vale \(\alpha\to1\) e la distribuzione gamma tende a un'esponenziale; questo ha l'effetto d'introdurre una pericolosa asimmetria nella densità, come si può notare nella \cref{figTransizioneAssimmetria}: una volta centrata rispetto alla media la moda (il valore piú frequente) ha valore negativo portando, in un orizzonte temporale finito, le perturbazioni negative a essere quelle piú frequenti.

                % \tikzfigure{htb}
                %     {\input{../../Figure/.fnl/Capitolo3/AssimmetriaGamma.tex}}
                \tikzexternalizedfigure{htb}
                    {../../Figure/.fnl/Capitolo3/AssimmetriaGamma.tex}
                    ...
                    {.775\linewidth}{figTransizioneAssimmetria}

                Il risultato è che con \(\sigma\) molto elevato le città tendono a spopolarsi perché v'è una preferenza per fluttuazioni negative. Per ovviare al problema bisogna ulteriormente vincolare \(\sigma\) affinché l'indice di asimmetria \(\rho\equiv2/\sqrt{\alpha}\) sia sufficientemente piccolo; dalla \cref{figTransizioneAssimmetria} si può perciò imporre \(\rho\leq0.2\), cioè
                %
                \begin{equation}
                    ...
                    \;\;\implies\;\;
                    \frac{(1-E)^2}{\sigma^2}\geq
                    \frac{(1-\lambda)^2}{\sigma^2}\geq100
                    \;\;\implies\;\;
                ...
                \begin{oss}
                    Quello qui mostrato è solo un possibile modello stocastico perturbativo; infatti, modificando le \cref{ipFluttuazioni}, se ne può scegliere un altro: ad esempio si può imporre una distribuzione uniforme, postulando una massima fluttuazione ammissibile, oppure si può troncare una gaussiana affinché soddisfaccia il vincolo di positività \cref{eqVincoloPositivitàFluttuazioni}.
                \end{oss}

                Non manca che caratterizzare il tasso d'immigrazione \(I\):

                \begin{ipo}[Conservazione della popolazione media]%[Caratterizzazione di \(I\)]
                    \label{ipConservazionePopolazioneMedia}
                    Si postula che le \ref{eqRegoleInterazioneUrbaneEsplicite} conservino [in media] la popolazione totale:
                    %
                    \begin{equation*}
                ...
                \begin{oss}
                    \label{ossUguaglianzaInterazioneEmigrazione}
                    Le \ref{eqRegoleInterazioneUrbaneEsplicite} non sono ancora complete siccome non si è esplicitata la regola d'emigrazione, la cui scelta, tuttavia, si può dunque vedere come la regola d'interazione stessa: è l'ultimo tassello del mosaico. Pertanto si lascia quest'ultima definizione al \cref{secRegoleEmigrazione} nel quale se ne propongono varie.
                \end{oss}
        ...
        \subsection{Equazione di tipo Boltzmann approssimata}\label{secApprossimazione}
            ...
            \begin{dfn}[Matrice dei gradi \(\mathbf B\)]
                ...
                Sia \(\mathbf B\in\mathbb R^{N\times N}\) la matrice di rango uno
                \[
            ...
            Con tale scelta è possibile recuperare un sistema di equazioni debole per le \(g_i\): usare, infatti, l'\cref{ipOsservabileUrbanoNodale} nella \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetricoUrbanoApprossimata} porta a
            %
            \begin{equation*}
                \begin{aligned}
                    ...
                    g^*_j\sum_{i\in\mathcal I}B(i,j)g_i\de s\de s_*,%\\&
            ...
            \end{equation}
            %
            ove \(\mathbf 1\equiv(1,1,\ldots,1)\in\mathbb R^N\), che altro non è che la densità media di \(S_t\) fra tutt'i vertici\footnotemark{}; quindi premoltiplicando la \cref{eqSistemaDensitàNodiUrbano} per \(\frac1N\mathbf 1^\top\) e usando la \cref{eqDensitàMarginaleUrbanaS} si ha%
            ...
            Eppure la \ref{eqDensitàMediaUrbanaS} non è ancora un'equazione chiusa per \(\bar g_N\) per via del secondo membro nel quale \(\mathbf g^\top\mathbf B\mathbf g_*\) richiede sia di conoscere nel dettaglio le connessioni sottostanti del grafo che le distribuzioni della popolazione di ogni città. Tuttavia, avvalendosi della definizione della \(\mathbf B\), la \ref{eqDensitàMediaUrbanaS} diventa
            %
            \begin{equation}
            ...
            \begin{dfn}[Densità dei gradi \(\mathbf k\)]
                ...
                Siano \(\mathcal K\equiv\{0,1,\ldots,N\}\) l'insieme dei gradi e 
                \[
                    d_N\colon\RPlus\times\mathcal K\times\RPlus\to\RPlus,
                \]
                ...
                \begin{align}
                    ...
                    =\sum_{k\in\mathcal K}\frac1N\sum_{i\in\mathcal I_k}g_i(s,t)
                    =\sum_{k\in\mathcal K}d_N(s,k,t),\label{eqDensitàGradiMarginale}\\
                    \mathbf k^\top\mathbf g(s,t)
                    &=\sum_{i\in\mathcal I}k_ig_i(s,t)
                    =\sum_{k\in\mathcal K}k\sum_{i\in\mathcal I_k}g_i(s,t)
                    =N\sum_{k\in\mathcal K}kd_N(s,k,t).\notag
                \end{align}
                %
            ...
            \begin{dfn}[Densità dei gradi \(\mathbf k\) normalizzati]
                ...
                Sia
                % \[
                %     \equiv\frac{k_i}N\in\underhat{\mathcal K},\quad\forall i\in\mathcal I,
                % \]
                \[
                    \underhat{\mathcal K}\equiv
                    \left\{\frac kN\ |\ k=0,1,\ldots,N\right\}
                    \subseteq[0,1]
                \]
            ...
            \begin{oss}
                ...
                Visto che due elementi consecutivi dell'insieme \(\underhat{\mathcal K}\) sono separati da un passo costante e uguale a \(\Delta\underhat k\equiv1/N\), che dalla \cref{eqDensitàGradiNormalizzati} implica
                \[
                ...
                \]
                cosa che permette di vedere la \(\underhat d_N\) come funzione costante a tratti dei gradi normalizzati \(\underhat k\), e similmente per \(d_N\) con \(\Delta k\equiv1\).% essendone un mero riscalamento.
            \end{oss}
            ...
            Con tutte queste definizioni e osservazioni, si può tornare alla \cref{eqPrimaIntroduzioneDensitàGradi} che può essere riformulare in termini di \(d_N\):
            %
            \begin{equation*}
                \derS{}t\sum_{k\in\mathcal K}\int_{\RPlus}\varphi d_N\de s=
                \sum_{k,k_*\in\mathcal K}^N\int_{\RPlus^2}\frac{kk_*}{D_N}
                \frac{\langle\varphi'+\varphi'_*-\varphi-\varphi_*\rangle}2
                d_Nd^*_N\de s\de s_*,
            \end{equation*}
            %
            dove \(k_*\) è il grado associato a \(g_N^*\), e poi in termini di \(\underhat d_N\) moltiplicando e dividendo per \(N\) il primo membro e per \(N^2\) il secondo:
            %
            \begin{equation*}
                \underbrace{
            ...
            \end{equation*}
            %
            si perviene infine a un'equazione di tipo Boltzmann analoga alla \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico}:
            %
            \eqtag{\([\text{EtB}]^k_S\)}{
            ...
            \tikzexternalizedfigure{tb}%h
                {../../Figure/.fnl/Capitolo3/SchemaRiassuntivoApprossimazione.tex}
                {Schema riassuntivo del \cref{teoRilassamentoTopologia}.} 
            ...
            Il significato del \cref{teoRilassamentoTopologia} è profondo: approssimare \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetricoUrbano} con \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetricoUrbanoApprossimata} coincide col perdere la distinguibilità indotta dal grafo rilassando in tal modo la topologia, la quale non scompare ma rimane solo come distribuzione dei gradi; tale riduzione è come sfocare i dettagli delle connessioni tra i vertici: si passa da uno specifico grafo a una classe di grafi aventi la stessa distribuzione dei gradi. Nella \cref{figSchemaRiassuntivoApprosimazione} è mostrato uno schema riassuntivo del \cref{teoRilassamentoTopologia} mentre nella \cref{figRappresentazioneGraficaApprossimazione} è raffigurata una sua rappresentazione piú grafica.

            % \tikzfigure{htb}
            %     {\input{../../Figure/.fnl/Capitolo3/RappresentazioneGraficaApprossimazione.tex}}
            \tikzexternalizedfigure{tb}%h
                {../../Figure/.fnl/Capitolo3/RappresentazioneGraficaApprossimazione.tex}
                {Rappresentazione grafica del \cref{teoRilassamentoTopologia}; per semplicità visiva si è disegnato un grafo indiretto, anziché il piú corretto diretto (\cref{ipTipoGrafo}), per la topologia approssimata e rilassata.}
                {.85\linewidth}{figRappresentazioneGraficaApprossimazione}
            ...
            \begin{oss}
                La \cref{figSchemaRiassuntivoApprosimazione} contiene al suo interno \(\underhat f\) che ha un'analoga definizione alla \cref{defDensitàGradiNormalizzata}, ma normalizzando gl'indici: sia%no
                % \[
                ...
                \]
                l'insieme discreto degl'indici normalizzati, allora la \cref{eqDensitàCongiuntaUrbana} normalizzata, per analogia colla \cref{eqDensitàGradiNormalizzati}, si può definire come
                \[
                ...
                \]
                anch'essa analoga alla \cref{eqDensitàGradiNormalizzati}, da cui
                \[
                ...
                \]
                Attraverso \(\underhat f\) e \(i=N\underhat i\) la densità marginale \(\bar f_N\) diventa
                \[
                    \bar f_N
                ...
                \]
                e, detto \(\Delta\underhat i\equiv1/N\) il passo tra due indici normalizzati consecutivi, si arriva a
                \[
            ...
            \begin{oss}
                Si veda \cite{Nurisso2024} per la dimostrazione del \cref{teoRilassamentoTopologia} in un contesto delle reti sociali e nel caso di un grafo diretto, non necessariamente simmetrico, oltre che con delle \ref{eqRegoleInterazione} lineari e simmetriche.
            \end{oss}

        \subsection{Altri nessi distinguibile-indistinguibile}\label{secNessiDI}

            Il \cref{teoRilassamentoTopologia} dimostra come si possa rilassare la struttura sottostante indotta dal grafo passando a una densità dei gradi normalizzati, recuperando cosí un'equazione classica di tipo Boltzmann. Risulta perciò stimolante esplorare se sussitano altre ipotesi che permettano di trasformare la \ref{eqDensitàMediaUrbanaS} in una forma analoga alla \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico}, approfondendo cosí i legami tra la teoria retale appena sviluppata e la teoria di tipo Boltzmann.
            ...
            \subsubsection{Ipotesi semplificative}
                ...
                \begin{ipo}
                    Esistono due principali ipotesi semplificative della \ref{eqDensitàMediaUrbanaS}:
            ...
            \subsubsection{Analisi della \texorpdfstring{\ref{ipMatriceUnitaria}}{S1}}
                ...
                Notando che \(\mathbf B=\mathbb1=\mathbf1\mathbf1^\top\), la \ref{eqDensitàMediaUrbanaS} diventa
                %
                \begin{equation*}
                    \begin{aligned}
                        \derS{}t\int_{\RPlus}\varphi\bar g_N\de s
                        &=\frac1{N^2}\int_{\RPlus^2}
                        \frac{\langle\varphi'+\varphi'_*-\varphi-\varphi_*\rangle}2
                        \mathbf g^\top\mathbf1\mathbf1^\top\mathbf g_*\de s\de s_*\\
                        &=\frac1{N^2}\int_{\RPlus^2}
                        \frac{\langle\varphi'+\varphi'_*-\varphi-\varphi_*\rangle}2
                        (\mathbf1^\top\mathbf g)(\mathbf1^\top\mathbf g_*)\de s\de s_*\\
                        &=\int_{\RPlus^2}
                        \frac{\langle\varphi'+\varphi'_*-\varphi-\varphi_*\rangle}2
                        \left(\frac1N\mathbf1^\top\mathbf g\right)
                        \left(\frac1N\mathbf1^\top\mathbf g_*\right)\de s\de s_*,
                    \end{aligned}
                \end{equation*}
                %
                ...
                \begin{equation}
                    ...
                    \bar g_N\bar g^*_N\de s\de s_*,
                \end{equation}
                %
                che è affine alla \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} ma con microstato \(\mathbf X_t\equiv S_t\) scalare, regole d'interazione \ref{eqRegoleInterazioneUrbane} e nucleo d'interazione unitario\footnotemark{} \(\mu\equiv1\) nella \ref{eqBernoulliInterazione}. Ciò significa che con \ref{ipMatriceUnitaria} gli agenti, nonostante siano distinti per il grafo, si possono vedere come indistinguibili purché si consideri la distribuzione media \eqref{eqDensitàMarginaleUrbanaS}.%
                \footnotetext{Tale risultato implica anche che un nucleo di collisione unitario nella \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} è il corrispettivo di una matrice unitaria nella \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetricoUrbanoApprossimata}.}%

            \subsubsection{Analisi della \texorpdfstring{\ref{ipAgentiIndistinguibili}}{S2}}

                Sotto tal'ipotesi, che equivale coll'assumere \(\mathbf g=\mathbf1g\) e \(\mathbf g_*=\mathbf1g_*\), la \ref{eqDensitàMediaUrbanaS} diventa
                %
                \begin{equation*}
                    \begin{aligned}
                        ...
                        &=\frac1{N^2}\int_{\RPlus^2}
                        \frac{\langle\varphi'+\varphi'_*-\varphi-\varphi_*\rangle}2
                        g\mathbf1^\top\mathbf B\mathbf1g_*\de s\de s_*\\
                        &=\frac{\mathbf1^\top\textbf B\mathbf1}{N^2}\int_{\RPlus^2}
                        \frac{\langle\varphi'+\varphi'_*-\varphi-\varphi_*\rangle}2
                        gg_*\de s\de s_*
                    \end{aligned}
                \end{equation*}
                %
                e rimembrando le \cref{eqNormaUnoMatriceAdiacenza,eqCriterioApprossimazioneA} si ottiene
                %
                \begin{equation}
                    \label{eqTipoBoltzmannConAgentiIndistinguibili}
                    \derS{}t\int_{\RPlus}\varphi g\de s=
                    \int_{\RPlus^2}\frac{D_N}{N^2}
                    \frac{\langle\varphi'-\varphi+\varphi'_*-\varphi_*\rangle}2
                    gg_*\de s\de s_*.
                \end{equation}
                %
                In questo contesto il rapporto \(D_N/N^2\in[0,1]\), che si ricorda essere il grado medio normalizzato \cref{eqGradoMedio}, rappresenta quanto la rete è topologicamente simile a una completamente connessa; difatti \(D_N\) è altrettanto interpretabile come il numero di lati presenti in un grafo diretto con limite superiore proprio \(N^2\), ossia il numero totale di coppie estraibili da \(N\) nodi.%
               
                Per il resto l'equazione è analoga alla \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} con microstato \(\mathbf X_t\equiv S_t\) scalare, regole d'interazione \ref{eqRegoleInterazioneUrbane} e nucleo d'interazione \(\mu\equiv D_N/N^2=\bar D_N\) nella \ref{eqBernoulliInterazione} costante. Dunque l'indistinguibilità degli agenti ha una notevole conseguenza sulla \ref{eqDensitàMediaUrbanaS}: essa permette di attribuire l'effetto complessivo del grafo al solo coefficiente \(\bar D_N\) il quale, dunque, ne rappresenta gli ultimi bagliori prima di una sua totale scomparsa per la \ref{ipMatriceUnitaria}.

            \subsubsection{Analisi delle \texorpdfstring{\ref{ipMatriceUnitaria} e \ref{ipAgentiIndistinguibili}}{S1 e S2}}

                Per la \ref{ipAgentiIndistinguibili} la densità media \cref{eqDensitàMarginaleUrbanaS} diventa 
                %
                \begin{equation*}
                    \bar g_N(s,t)=\frac1N\sum_{i\in\mathcal I}g_i(s,t)
                    \overset{\ref{ipAgentiIndistinguibili}}=
                    \frac1N\sum_{i\in\mathcal I}g(s,t)=g(s,t),
                \end{equation*}
                %
                ossia \(\bar g_N\) coincide con quella di tutti gli agenti \(g\), essendo questi, appunto, indistinguibili. Allora, dalla \ref{ipMatriceUnitaria}, la \eqref{eqTipoBoltzmannConGrafoUnitario} diventa
                %
                \begin{equation}
                    \label{eqTipoBoltzmannConGrafoUnitarioEAgentiIndistinguibili}
                    \derS{}t\int_{\RPlus}\varphi g\de s=\int_{\RPlus^2}
                    \frac{\langle\varphi'-\varphi+\varphi'_*-\varphi_*\rangle}2
                    gg_*\de s\de s_*,
                \end{equation}
                %
                che è uguale in tutto e per tutto alla \ref{eqFormaDeboleTipoBoltzmannOmogeneoAssimmetrico} con microstato \(\mathbf X_t\equiv S_t\) scalare, regole d'interazione \ref{eqRegoleInterazioneUrbane} e nucleo d'interazione unitario \(\mu\equiv1\) nella \ref{eqBernoulliInterazione}. In effetti, contrariamente alle \cref{eqTipoBoltzmannConGrafoUnitario}, con \(\bar g_N\), e \cref{eqTipoBoltzmannConAgentiIndistinguibili}, con \(\bar D_N\), il grafo è completamente scomparso dalla \cref{eqTipoBoltzmannConGrafoUnitarioEAgentiIndistinguibili}.
                %
                ...
                % \begin{equation*}
                %     \int_{\RPlus^2}
                %     \langle\varphi_*'-\varphi_*\rangle gg_*dsds_*
                %     =\int_{\RPlus^2}
                %     \langle\varphi'-\varphi\rangle gg^*dsds_*,
                % \end{equation*}
                % %
                % dalla quale si ha
                % %
                % \begin{equation*}
                %     \derS{}t\int_{\RPlus}\varphi gds=
                %     \int_{\RPlus^2}
                %     \langle\varphi'-\varphi\rangle gg^*dsds_*.
                % \end{equation*}
                % %
