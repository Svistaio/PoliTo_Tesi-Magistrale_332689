...
\chapter[Teoria cinetica dei sistemi multiagente]{\texorpdfstring
    ...
    \section{Definizioni preliminari di probabilità}\label{secDefinizioniPreliminariProbabilitàTCSMA}
        ...
        \begin{oss}[Dalla teoria alla pratica]
            ...
            Esiste un'interpretazione piú pratica per l'attesa condizionata rispetto alla \cref{defValoreAttesoCondizionato} alquanto teorica: la \(\mathbb E[\mathbf X|\mathcal G]\) si calcola mediando \(\mathbf X\) \textit{considerando \(\mathcal G\) noto}; ciò implica che si sanno tutte le informazioni di \(\mathcal G\), ovverosia che i suoi eventi si sono realizzati.
           
            Un uso che permette di capire meglio quanto detto si può trovare nell'\ref{ipAttesaCondizionataUgualeMisura} imponendo \(A=\Omega\) (sempre lecito essendo \(\mathcal G\) una \(\sigma\)-al\-ge\-bra):
            %
            \begin{equation}
                \label{eqAttesaCondizionataInterpretazionePraticaGenerale}
                \mathbb E[\mathbf X]
                =\mathbb E[\mathbb E[\mathbf X|\mathcal G]],
            \end{equation}
            %
            che a parole significa che l'attesa di \(\mathbf X\) si può calcolare mediando l'attesa condizionata; tuttavia \(\mathcal G\) continua a essere una generica \(\sigma\)-algebra, difficilmente rappresentabile, per cui è opportuno specializzare ulteriormente la \cref{eqAttesaCondizionataInterpretazionePraticaGenerale}.
            ...
            \begin{enumerate}[
                ...
                \item un vettore aleatorio \(\mathbf Z=(\mathbf X,\mathbf Y)\in\mathbb R^n\), con \(\mathbf X\in\mathbb R^h\) e \(\mathbf Y\in\mathbb R^k\) discreta, e% con densità congiunta \(f_{\mathbf Z}(\mathbf x,\mathbf y)\) e marginali \(f_{\mathbf X}\) e \(f_{\mathbf Y}\), e
            ...
            allora, ipotizzando che \(\mathbf X(\mathbf Y)\) sia funzione di \(\mathbf Y\) e che quest'ultimo \textit{non} sia funzione\footnotemark{} di \(\mathbf X\), posto \(\mathcal G\equiv\mathbf Y\) la \cref{eqAttesaCondizionataInterpretazionePraticaGenerale} diventa%
            ...
            \end{equation*}
            %
            ma ora è facile capire cosa s'intende per \(\mathbf Y\) noto: si assuma che \(\mathbf Y\) abbia realizzato l'evento \(\mathbf y\) arbitrario, e, siccome non è funzione di \(\mathbf X\), ciò implica
            %
            \begin{equation}
        ...
        \subsection{Equazione di tipo Boltzmann esatta}\label{secTipoBoltzmannUrbano}
            ...
            \subsubsection{Descrizione e derivazione}
                ...
                \tikzexternalizedfigure{htb}
                    ...
                    {Rappresentazione grafica dell'impostazione teorica della \protect\TCSMA{} su reti.}
                    {.8\linewidth}{figRappresentazioneGraficaTCSMAReti}
                ...
                ove \(A\colon\mathcal I\times\mathcal I\to\mathbb R\) è una funzione che a ogni coppia d'indici \((I,I_*)\) associa la relativa entrata nella matrice d'adiacenza
                %
                \begin{equation*}
            ...
            \subsubsection{Analisi delle regole d'interazione}
                ...
                \begin{ipo}
                    ...
                    \end{equation*}
                    %
                    D'altra parte il postato \(s_*'\) [per il momento] non si considera poiché è per definizione sempre positivo qualunque sia il tasso d'immigrazione \(I\in\RPlus\).
                \end{ipo}
                ...
                \end{equation}
                %
                La scelta di \(>\) anziché \(\geq\) nella \eqref{eqVincoloPositivitàFluttuazioni} è ben fondata: di fatto si sta supponendo che le fluttuazioni non possano spopolare una città\footnotemark{}; tale condizione discende dal rapporto \(s_*/s\) contenuto nelle regole d'emigrazione proposte nel \cref{secRegoleEmigrazione}.%
                \footnotetext{Ciò non significa che il modello non possa prevedere un tale fenomeno, poiché la taglia può arbitrariamente avvicinarsi a zero ma mai esserne uguale, raggiungendolo solo \textit{a posteriori} dopo l'approssimazione dai numeri reali a quelli interi (\cref{ossNaturaNumericaPopolazione}).}%
