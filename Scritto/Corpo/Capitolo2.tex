...
\chapter{Note di teoria dei grafi}\label{secTeoriaGrafi}
    ...
    \section{Definizioni miscellanee}\label{secDefinizioniGrafi}
        ...
        \tikzfigure{H}{.8\linewidth}
            ...
            {1.2\linewidth}{figEsempioGrafoInDiretto}

        \begin{dfn}[Matrice d'adiacenza]
            \label{defMatriceAdiacenzaUnitariaPesata}
            \newcommand\hspz{\hspace{1em}}
            Sia \(N\equiv\vert\mathcal I\vert\in\NPlus\) la cardinalità dell'insieme degl'indici, ossia il numero di vertici totali, si definisce la matrice d'adiacenza \(\mathbf W\in\mathbb R^{N\times N}\) pesata come:
            %
            \begin{equation}
        ...
        \begin{dfn}[Matrice trasposta]
            Data \(\mathbf A\in\mathbb R^{N\times N}\), con \(N\in\NPlus\), allora s'indica \(\mathbf A^\top\) la matrice trasposta cosí definita:
            %
            \begin{equation*}
                a^\top_{i,j}=a_{j,i}\quad\forall i,j\in\{1,2,\ldots,N\}.
            \end{equation*}
        \end{dfn}

        \begin{dfn}[Matrice simmetrica]
            Data \(\mathbf A\in\mathbb R^{N\times N}\), con \(N\in\NPlus\), allora
            %
            \begin{equation}
                \label{eqMatriceAdiacenzaSimmetrica}
                \mathbf A\text{ è simmetrica}
                \ \ \iff\ \ 
                a_{i,j}=a_{j,i}=a^\top_{i,j}
                \ \ \iff\ \ 
