...
\chapter{Note di teoria dei grafi}\label{secTeoriaGrafi}
    ...
    \section{Definizioni miscellanee}\label{secDefinizioniGrafi}
        ...
        La trascurabilità della direzione sarà discussa poco dopo nella \cref{secRetiECittà}, anche se è facilmente intuibile dall'esempio.
        
        \begin{dfn}[Matrice d'adiacenza]
            \label{defMatriceAdiacenzaUnitariaPesata}
            \newcommand\hspz{\hspace{1em}}
            Sia \(N\equiv\vert\mathcal I\vert\) la cardinalità dell'insieme degl'indici, ossia il numero di vertici totali, si definisce la matrice d'adiacenza \(\mathbf W\in\mathbb R^{N\times N}\) pesata come:
            %
            \begin{equation}
            ...
            \end{equation}
            %
            ove \(q_{i,j}\) è il peso associato al lato \((i,j)\); nel caso in cui \(q_{i,j}\equiv1\) \(\forall i,j\in\mathcal I\) si ha la matrice d'adiacenza unitaria \(\mathbf A\in\mathbb R^{N\times N}\):
            %
            \begin{equation}
            ...
            \end{equation}
            %
            Si osserva che, se non altrimenti detto, scrivendo solo «matrice d'adiacenza» si sottintende sempre l'unitarietà.
        \end{dfn}

        \begin{oss}
            Per i grafi indiretti sia \(\mathbf W\) che \(\mathbf A\) sono simmetriche mentre per quelli diretti non è detto che sia cosí.
        \end{oss}
        ...
        \begin{dfn}[Grado e Forza \textnormal{[entrate/uscente]}]
            ...
            \end{equation}
            %
            d'altra parte in un grafo diretto non pesato la seconda equivalenza non è piú [necessariamente] valida, ragion per cui è necessario specializzare il grado entrante e uscente 
            %
            \begin{equation}
                \label{eqGradoForzaGrafoDiretto}
                k^-_i\equiv\sum_{j=1}^{\vert\mathcal I\vert}a_{j,i}
                \quad\text{e}\quad
                k^+_i\equiv\sum_{j=1}^{\vert\mathcal I\vert}a_{i,j},
            \end{equation}
            %
            rispettivamente.
           
            Se il grafo è invece pesato le definizioni sono le stesse di \cref{eqGradoForzaGrafoIndiretto,eqGradoForzaGrafoDiretto} ma con \(w_{i,j}/w_{j,i}\) in luogo di \(a_{i,j}/a_{j,i}\).
        \end{dfn}
