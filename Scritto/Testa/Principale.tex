...
\begin{document}
    ...
    % \part{Teoria}
    \chapter[Teorie cinetiche dei sistemi multiagente]{\texorpdfstring
        ...
        \section{Derivazione dell'equazioni cinetiche}
            ...
            \subsection{Derivazione esatta}
                ...
                Una descrizione cinetica dell'algoritmo \refAlgIntAR{} coincide con dell'equazioni d'evoluzione per le distribuzioni di probabiltà \(f_i\) delle opinioni degli agenti; per derivarle si procede mediante un metodo classico nella teoria dei sistemi multiagente \cite{Nurisso2024,IMS2103}.
            ...
            \subsection{Interazioni tra città}
                ...
                \begin{equation}
                    ...
                    \left\{\begin{aligned}
                        s'&=s\bigl(1-E(s,s_*)+\mu\bigr)\\
                        s_*'&=s_*+sI(s,s_*)
                    \end{aligned}\right.
                \end{equation}
                %
                ove \(s\) e \(s_*\) sono le città interagente e subente rispettivamente, \(E(s,s_*)\) e \(I(s,s_*)\) sono rispettivamente i tassi di emigrazione e immigrazione, mentre \(\mu\) rappresenta fluttuazioni stocastiche del tipo
                %
                \begin{equation}
                    \label{eqTipoFluttuazioni}
                    \langle\mu\rangle=0
                    \quad\text{e}\quad
                    \langle\mu^2\rangle=\sigma,
                \end{equation}
                %
                con \(\sigma\) uguale al valore della varianza; rispetto alle leggi d'interazioni lineari \eqref{eqStatiPostInterazioniLineari} le \eqref{eqSchemaInterazioneCittà} soddisfano
                %
                \begin{equation}
                    \label{eqLegameLineareInterazioneCittà}
                    % \begin{aligned}
                    %     p(s,s_*)&\equiv s[1-E(s,s_*)+\mu]\\
                    %     p_*(s,s_*)&\equiv s_*
                    % \end{aligned}
                    % \begin{aligned}
                    %     \quad&\text{e}\quad\\
                ...
                \end{equation}
                %
                ciò ha senso perché l'emigrazione e l'immigrazione sono fenomeni relativi (invertendo \(s\) ed \(s_*\) sarebbe l'opposto). La scelta di \(E(s,s_*)\) dipende da come si vuole modellizzare il fenomeno dell'immigrazione, e in questo manuscritto si è modificata la \cite[(2.2), § 2, p. 223]{Gualandi2019} mediante la \cite[(4.5), § 4, p. 228]{Gualandi2019}: 
                %
                \begin{equation}
                ...
                \end{equation}
                %
                che in essenza è una funzione di Hill di ordine \(\alpha\), in cui la logica è che v'è un tasso di emigrazione maggiore verso città con popolazione relativa maggiore, dato dal rapporto \(s_*/s\), con un massimo \(\lambda\in(0,1)\); in poche parole la \eqref{eqLeggeEmigrazione} descrive la tendenza degl'individui di aggregarsi per i piú svariati motivi: lavoro, sicurezza, famiglia, eccetera.
                ...
                Non manca che caratterizzare il tipo di perturbazione \(\mu\) per avere uno stato postinterazione fisicamente sensato; difatti, è chiaro che rigorosamente \(\mathcal P\equiv\mathbb N\) ma è piú agevole supporre \(\mathcal P\equiv\mathbb R^+\) per poi approssimare per eccesso o difetto il numero reale effettivo; pertanto, dalla \eqref{eqSchemaInterazioneCittà}, si ha
                %
                \begin{equation}
                    \label{eqPositivitàFluttuazioni}
                    s'>0\;\;\implies\;\;\mu>E(s,s_*)-1
                \end{equation}
                %
                mentre \(s_*'\) è per definizione sempre positivo.
