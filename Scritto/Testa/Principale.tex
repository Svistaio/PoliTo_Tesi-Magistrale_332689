    ...
    \chapter[Teorie cinetiche dei sistemi multiagente]{\texorpdfstring
        ...
        \section{Descrizione cinetica retale}\label{secDescrizioneCinetica} %  d'interazioni mediante da un grafo
            ...
            \subsection{Interazioni azione-reazione}
                ...
                \end{equation*}
                %
                è definita mediante una funzione \(\Psi_*:\mathcal P^2\times\Omega\to\mathcal P\) potenzialmente diversa da \(\Psi\).

                Questo tipo d'interazione, prendendo come riferimento \cite[§ 2.2.1]{Nurisso2024} sarà identificato come azione-reazione, e riassunto dall'algoritmo
                %
                \begin{equation*}
        ...
        \section{Derivazione dell'equazioni cinetiche}
            ...
            \subsection{Derivazione esatta}
                ...
                Sia \(\Phi\equiv\Phi(x,s):\mathcal I\times\mathcal P\to\mathbb R\) un osservabile arbitrario (funzione \textit{test}), cioè una quantità che si può calcolata sapendo lo stato microscopico di un generico agente rappresentativo del sistema. Allora dalla prima equazione in \refAlgIntAR{} valutando il valore atteso dell'osservabile postinterazione, si ha
                %
                \begin{equation*}
                    %\label{}
                    \begin{aligned}
                        \mathbb E[\Phi(X,V_{t+\Delta t})]=
                        \mathbb E\left[\mathbb E\right.[
                            &\Phi\left(X,(1-\Theta)V_t
                            +\Theta\Psi(V_t,V_t^*,\omega)
                            A(X,X_*)\Delta t\right)|X,X_*
                        ]\left.\vphantom{\mathbb E}\right]\\
                        =\mathbb E\left[
                            \mathbb E\right.[
                            &\Phi(X,V_t)(1-A(X,X_*)\Delta t)
                            \\+&\Phi\left(
                                X,\Psi(V_t,V_t^*,\omega)
                ...
                \begin{equation*}
                    ...
                    \mathbb E\left[
                        A(X,X_*)\left(
                            \Phi(X,\Psi(V_t,V_t^*,\omega))-\Phi(X,V_t)
                        \right)
                    \right],
                \end{equation*}
                %
                ...
                %
                \begin{equation}
                    \label{eqDinamicaValoreAtteso}
                    \derS{\mathbb E[\Phi(X,V_t)]}t=
                    \mathbb E\left[
                        A(X,X^*)\left(
                            \Phi(X,\Psi(V_t,V_t^*,\omega))-\Phi(X,V_t)
                        \right)
